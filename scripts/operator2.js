...
// ++, --
a = d--; //a에 d대입 후 d에 -1 / a는기존1없어지고 d의값인4대입, 그 뒤에 d는4-1=3
...
num1++; //10+1 //대입연산자가 없으면 증감연산자 앞뒤 의미없음
num2--; //20-1
num3++; //30+1
num4--; //40-1
console.log(num1, num2, num3, num4)
//num1=11, num2=19, num3=31, num4=39
num1 = ++num2; //19+1=20 //전위 
num2 = num1++; //20+1 //후위 num1은 num2대입이후 본인에게 1더해서 21
console.log(num1, num2, num3, num4)
...
//num++시 증가 전 값이 plus 인식 -> 전위연산 ++num
//let num = Number(prompt('구입 수량을 입력하세요.')); //const 변경 불가
...
//복합대입연산자는 새로운값이아닌 기존값을 활용해서 추가대입
//키 140 -> 145 
let height = 140;
//height = 5; //X (새로운 값 5만 대입됨)
//height = height + 5; //기존 140에 +5를 해서 대입
...
wallet1 += month; //wallet = wallet + month;
let total = `용돈을 더해서 총 ${wallet1}원이 되었습니다.`;
console.log(total);

//지갑 10000 준비
//편의점에서 물건을 구입할때 지갑의 돈이 나가는 상황
//삼각김밥 2000, 아이스크림 3000, 과자 4000, 남은 돈
//결과형태) 편의점 총 구입비는 원이고 남은돈은 원입니다.
let wallet = 10000;
let kimbab = 2000;
let icecream = 3000;
let snack = 4000;
...
//wallet -= kimbab;
//result = `현재 남은 돈은 ${total}원`
wallet -= total;
result = `편의점 총 구입비는 ${total.toLocaleString(ko-kr)}원이고 남은 돈은 ${wallet.toLocaleString(ko-kr)}원입니다.`;
console.log(result);