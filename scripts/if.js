...
//1. typeResult 변수 생성
//2. Boolean내장함수 활용 data1 변수에 들어간 논리데이터종류확인
//3. typeResult 대입
...
console.log(`data2의 데이터타입은 : ${typeResult}`);

let data3 = false; //fallse (논리키워드.문자열아님.바로작성가능)
typeResult = Boolean(data3);
console.log(`data3의 데이터타입은 : ${typeResult}`);

data3 = null; //null 값을 삭제할때 대입하는 키워드
typeResult = Boolean(data3);
console.log(`data3의 데이터타입은 : ${typeResult}`);

data3 = data2 + 1; //NaN (Not a Number)
console.log(data3);
typeResult = Boolean(data3);
console.log(`data3의 데이터타입은 : ${typeResult}`);

data2 = [0,1,2]; //3 배열
data3 = []; //빈배열 new Array(); 내장함수가 있다로 구분. 참
//빈 문자열''은 거짓
typeResult = Boolean(data2);
console.log(`data2의 데이터타입은 : ${typeResult}`);
typeResult = Boolean(data3);
...
let x = 10;
let y = 20;
let result = x != y; //=(대입) ==(피연산자가 같은지 비교연산) !=(다른지 비교)
...
prtBtn.addEventListener('click', function(){
    ...
    //if(user==true) print();
    //if(user!=false) print();
    if(user) print(); //user에 유저행동에따라 참/거짓 대입됨 -> 참일시 print실행
})
...
adBtn.addEventListener('click', function(){
    ...
    else alert('성인이 아니면 접속이 불가합니다.');
    // if(user){ //2줄 이상일땐 중괄호 처리하기
    //     location.href = 'https://google.com';
    //     console.log(user);
    // }else{
    //     alert('성인이 아니면 접속이 불가합니다.');
    //     }
})