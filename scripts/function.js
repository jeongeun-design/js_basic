...
// 함수호출
func_z(); // 1+undefined NaN == Not a Number 약자(숫자가 아니라 계산할 수 없음)
...
//함수호출
back(); //결과는? 지폐 지역변수값
console.log(`wallet : ${wallet}`); //결과는? 동전 전역변수값
...
house(); //매개변수의 기본값이 있을 때 호출 괄호는 비워도됨
house('빵부스러기');
...
cafeKiosk('따뜻한', 2, 2);
cafeKiosk(undefined, 1);
function cafeKiosk(temperature='차가운', menu, num=1){
